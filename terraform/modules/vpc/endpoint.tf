...
resource "aws_vpc_endpoint" "tagging" {
  count             = var.enable_tagging_endpoint ? 1 : 0
  vpc_id            = aws_vpc.this.id
  service_name      = "com.amazonaws.${var.region}.tagging"
  vpc_endpoint_type = "Interface"
  ...
  tags = merge(var.shared_tags, { Name = "${var.name}-tagging-endpoint" })
}

resource "aws_security_group" "vpc_endpoints" {
  count       = (var.enable_cloudwatch_endpoint || var.enable_sts_endpoint || var.enable_tagging_endpoint) ? 1 : 0
