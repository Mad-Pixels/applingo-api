...
func (r *DictionaryCheckData) GetFilename() string {
	return r.filename
}
...
func (r *DictionaryCheckData) Setup(ctx context.Context, req RequestDictionaryCheck, s3cli *cloud.Bucket, promptBucketName, dictionaryBucketName string) error {
	var (
	...
	// filename.
	if req.DictionaryFile == "" {
		return ErrorDictionaryFileIsRequired
	}
	r.filename = req.DictionaryFile
	// prompt.
	...
	runWorker(ctx, &wg, results, "dictionary", func() error {
		...
		if err := s3cli.Read(ctx, r.dictionaryBuf, r.filename, dictionaryBucketName); err != nil {
			return errors.Join(ErrorGetBucketFileContent(r.filename, dictionaryBucketName), err)
		}
		return nil
	})
